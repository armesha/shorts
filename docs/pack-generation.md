...
- каждый мем обязательно смотреть как изображение целиком: учитывать картинку, персонажей, подписи,
  расположение текста, выражения лиц и визуальный панчлайн. OCR допустим только как помощь при
  расшифровке; нельзя выбирать комментарий, эмоцию и подачу только по извлечённому тексту;
- до расшифровки, TTS и рендера каждый мем обязательно проходит единый visual safety + rights gate.
  Отбраковывать водяные знаки, неясные права, кадры фильмов/сериалов/мультфильмов/аниме/игр,
  узнаваемых знаменитостей и реальных людей как объект насмешки, защищённых персонажей/бренды и
  другие явные copyright/reused-content риски. Одновременно отбраковывать адресные оскорбления и
  унижение, травлю, protected-class slurs/стереотипы и hate speech, сексуальный контент, угрозы,
  графическое насилие, поощрение или шутливую нормализацию суицида/самоповреждения и другие явные
  риски правил YouTube. Мат, мрачная самоирония, подразумеваемый вред и неоднозначный контекст считать
  `borderline`: не отправлять в озвучку и рендер автоматически, пока не принято отдельное решение;
- большие архивы делить примерно по 50 изображений на vision-субагента. Каждый субагент открывает и
  визуально проверяет все назначенные файлы, возвращая по каждому точный текст, краткое описание
  изображения, решение о вступительном комментарии, актёрскую подачу, статус `accept` / `borderline` /
  `reject` по правам и safety и конкретную причину отбраковки/ручной проверки при наличии.
  Основной агент объединяет результаты по исходным именам файлов и проверяет согласованность перед TTS;
- читать текст мема как написано; не добавлять новые шутки и реплики без отдельной просьбы;
- если пользователь разрешил авторский комментарий и сам мем очень короткий, перед чтением допустима
  одна короткая естественная реплика о том, что видно на изображении. Она должна называть смешную
  визуальную деталь простыми разговорными словами и мягко подводить к тексту мема, а не объяснять
  шутку и не придумывать новый сюжет. Для длинного или уже самодостаточного текста комментарий не
  добавлять. После комментария читать оригинальный текст мема без изменений;
- сначала разобрать смысловые биты: спокойная завязка, пауза, панчлайн, возможный смех/вздох;
- не использовать для Витька одну и ту же подачу во всех роликах. До TTS анализировать конкретный
  мем и выбирать подходящую игру: где-то спокойнее или медленнее, где-то быстрее/возбуждённее,
  где-то со вздохом, нехваткой воздуха, недоверием, шёпотом или смехом. Эти приёмы добавляют только
  как актёрскую разметку и не переписывают текст мема;
...
- для динамичного заднего плана можно использовать локальный длинный gameplay-футаж без исходного
  звука. Фрагмент выбирать автоматически под длительность Shorts с историей диапазонов: сначала
  использовать участки с нулевым пересечением, затем с минимальным overlap score; выбранные интервалы
  сохранять в runtime-history, чтобы одни и те же моменты повторялись как можно реже;
- внутри круга персонажа использовать отдельный спокойный интерьерный фон, подходящий персонажу.
  Центральную область держать малодетальной, волосы, голову и плечи показывать целиком;
- фон внутри круга и цвет ободка выбирать независимо из project-owned пулов через
  `scripts/select-avatar-style.mjs`. История хранит счётчики использования: сначала должны пройти
  все доступные варианты, затем начинается новый равномерный цикл. Не закреплять один фон или один
  цвет за персонажем навсегда;
- если в самом меме есть очевидная шутка и смех усиливает панчлайн, иногда допустимо короткое
  хихиканье после текста. Не добавлять его в каждый ролик. Для Gemini 3.1 Flash TTS использовать
  документированный `[laughing]` и конкретно описывать в style prompt характер смеха; произвольные
  непроверенные теги не использовать. Мимику смеха синхронизировать по фактическому WAV;
- для автоматического подбора включать в Audio Lab режим «Индивидуальная реакция на мем». Он сначала
  разбирает конкретный текст на завязку, развитие и панчлайн, затем выбирает не больше двух совместимых
  приёмов: сухой финал, улыбку в голосе, короткий смешок, выдох недоверия, усталый вздох, удивлённый
  вдох, сомневающуюся паузу, неловкое молчание, короткий шёпот, нарастающее недоверие, сдержанное
  возбуждение, нарочитую серьёзность, мягкое сочувствие или усиление последней фразы. Часто достаточно
  одного приёма; если реакция не усиливает шутку, оставлять сухой финал. Режим не меняет слова мема,
  а явно поставленные теги имеют приоритет;
...
Первый prebuilt-пак этого формата — `voiced-memes-ru` («Озвучка мемов»):

- реестр: `src/anecdotes/decks.ts`, `web/src/lib/deck.ts`;
- список: `data/voiced-memes-ru/videos.json`, provenance: `data/voiced-memes-ru/sources.json`;
- runtime MP4: `assets/fact-videos/voiced-memes-ru/`;
- админ-превью: pack `voiced-memes-ru` в `data/output/admin-demos/manifest.json`;
- публикация текущего одобренного ролика: `node scripts/publish-voiced-memes-pack.mjs [готовый.mp4]`.
  Скрипт копирует MP4, делает постер и точечно обновляет только этот pack в manifest.

### Пул динамичных фонов для `voiced-memes-ru`

Фоны берутся только из вертикальных роликов канала, который пользователь явно выбрал и подтвердил для
использования: `https://www.youtube.com/@OrbitalNCG/videos`. Не возвращать прежние Minecraft/Subway/Roblox/
GTA/slime-файлы из других источников и не добавлять сторонние YouTube-ссылки без отдельного подтверждения
пользователя.

- реестр и provenance: `data/voiced-memes-ru/gameplay-sources.json`;
- локальные видео без исходного звука: `assets/fact-videos/voiced-memes-ru/sources/orbital-*.mp4`;
- синхронизация всех вертикальных роликов канала: `node scripts/sync-voiced-memes-gameplay-pool.mjs --download`;
- selector сначала равномерно выбирает **тип** (`minecraft`, `subway-surfers`, `gta`, `roblox`, `forza`),
  а затем случайно выбирает наименее использованный ролик внутри этого типа. Два Minecraft-ролика не
  увеличивают суммарную вероятность Minecraft;
- затем `scripts/select-gameplay-segment.mjs` выбирает у ролика фрагмент с минимальным пересечением с
  историей. Никогда не выбирать один и тот же кусок систематически;
- брать только готовые MP4, не `.part`; исходный звук в ролики не мьюксить.

После пополнения проверить пул:

```bash
node scripts/sync-voiced-memes-gameplay-pool.mjs
```

Перезапуск сервера не нужен: это локальные assets и скрипты рендера. Перед пересборкой уже готовых
Shorts очистить их render-progress и истории только после явного решения пользователя пересобрать весь
набор с новым пулом.
...
> Архивный workflow. 27.08.2026 все варианты `visual-riddles*` выведены из активного каталога и
> удалены с production-диска как не подключённые ни к одному аккаунту, библиотеке или очереди.
> Этот раздел сохранён только как история сборки; перед возвратом пака нужны повторная регистрация,
> восстановление licensed assets и полная проверка.
...
> С 27.08.2026 активным остаётся только RU-пак `illusions-3d`. Варианты `illusions-3d-de` и
> `illusions-3d-en` удалены с production-диска и из каталога как неиспользуемые.
...
- синхронизация уже созданных карточек библиотеки после повторного импорта:
  `node --experimental-sqlite scripts/sync-shortrobot1-library.mjs <accountId>` (каналы `106` и `107`;
  обновляет MP4 и постеры на месте, не создавая дубликаты и не возвращая уже опубликованные ролики в
  очередь); канал 107 использует отдельное расписание и seeded-выбор из всей свободной библиотеки,
  поэтому публикует ролики не по порядковым номерам;
...
Публичный заголовок каждого ролика фиксирован: `@shortrobot — Telegram-бот 😂 #shorts`; внутренние
порядковые названия выпусков остаются только в библиотеке для удобства управления.
...
- UI: `web/public/circle-editor/`, отдельный экран: `web/src/pages/CircleEditor.tsx`
  по адресу `/circles`; библиотека баннеров для всех авторизованных пользователей находится на `/admin/banners`.
- API-мост: `server/routes/circle-editor.ts`; редактор, загрузка кружков и рендер доступны всем
  авторизованным пользователям, как и просмотр, создание и удаление рекламных баннеров.
- Рабочие настройки и загруженные файлы по умолчанию хранятся в gitignored
  `data/telegram-circles/`; при первом обращении сервер сам создаёт `config.json` и рабочие каталоги.
  Для совместимости отдельный каталог по-прежнему можно задать через `TG_CIRCLES_DIR`.
- Раскладка сохраняется в рабочем `config.json`: координаты и размеры кружка, загадки и баннера.
- Активный баннер сохраняется одновременно в `config.json` и активном шаблоне
  `circle-templates.json`. Выбор через редактор переживает обновление страницы и перезапуск сервера;
  установочный скрипт меняет его только при явном флаге `--activate`.
...
- Пользователь может загрузить собственный MP4/MOV/WebM/MKV/M4V до 500 МБ прямо в редакторе; сервер
  ...
  и добавляет его только в библиотеку этого владельца. Лимит загрузки через облачный Bot API — 20 МБ.
- Режим кружка «Случайный — без повторов» хранит отдельную ротацию владельца в
  `.runtime/source-rotation-u<userId>.json`: каждый доступный этому пользователю кружок используется
  один раз за цикл. Новые ручные и Telegram-загрузки имеют префикс владельца; старые файлы без префикса
  остаются общими для обратной совместимости.
...
- Любой выбранный анимированный баннер автоматически повторяется с исходной скоростью до конца
  Telegram-кружка и обрезается ровно по длительности итогового видео. Для отдельной заранее
  подогнанной MOV-копии есть CLI:
  `npm run circle:banner:fit -- --banner <banner.mov> --video <video.mp4> --output <fitted.mov>`.
- Локальный пользовательский баннер устанавливается воспроизводимо через
  `npm run circle:banner:install -- --source <banner.mov> --id <id> --name <название> --activate`;
  скрипт проверяет видео, сохраняет provenance рядом с ассетом и при `--activate` обновляет активный
  шаблон. Без `--activate` текущий выбор пользователя не меняется.
