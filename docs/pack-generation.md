...
и `funny_quotes` с весом 1. Это не жесткий порядок слотов, а целевое взвешивание: генерация и добивка
стремятся к пропорции на длинной дистанции, но порядок роликов может рандомизироваться между каналами.
Длинные видео и visual-riddle деки в эту формулу не включать: длинные сборники публикуются отдельно
из библиотеки канала, а `Вижу Ответ` остается тематикой блока `riddles_illusions`.
