...
- Кнопка генерации запускает `tg circles/src/render-cli.ts`; Telegram-кружок берётся из `downloads/`,
  gameplay — из `gameplay/`, результат — из `output/`. Пользователь может загрузить собственный
  MP4/MOV/WebM/MKV/M4V до 500 МБ прямо в редакторе; сервер проверяет, что файл содержит читаемый видеопоток.
