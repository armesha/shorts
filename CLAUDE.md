...
## 🚫 Ассеты — ТОЛЬКО «без ап» / no-copyright (MANDATORY)
- **ВСЕ визуальные И звуковые ассеты любой деки/пака/видео — строго copyright-free** («без ап» =
  без авторских прав / royalty-free). Разрешено ТОЛЬКО: **Pexels** (Pexels License — free commercial,
  без атрибуции), **CC0 / Public Domain** (Wikimedia/Openverse и т.п.) или **собственная генерация**.
  По каждому ассету фиксируй источник+лицензию (образец — `data/memes/photos/sources.jsonl`).
- **НИКОГДА не используй чужой копирайт:** кадры из фильмов/сериалов/игр/аниме, знаменитые
  мем-шаблоны (Drake, Distracted Boyfriend, Bernie, Spongebob и т.п.), чужой арт,
  платный сток, «народные» мемы с владельцем (Ждун/Превед). **Музыка/звук — только CC0/PD/no-attribution**
  (НЕ Pixabay — вето пользователя; и помни: музыкальный Content ID ловится автоматически и
  блокирует/глушит видео по регионам).
- **Фото знаменитостей (уточнение 2026-07-08):** критерий тот же, что и для любого другого ассета —
  лицензия, а не тема кадра. Обычный пресс-снимок/папарацци/промо-кадр студии, «просто найденный в
  поиске» — это ВСЕГДА чужой копирайт. Разрешено ТОЛЬКО конкретное фото с явной и проверенной
  PD/CC0/CC-BY лицензией (корректный тег на Wikimedia Commons, официальный госPD-архив и т.п.) —
  с обязательной фиксацией источника+лицензии, как для любого другого ассета. На практике это почти
  всегда означает фигуры, умершие достаточно давно (обычно 70+ лет), либо явно PD-релизнутые
  архивные/госфото — для большинства ныне живых или недавних знаменитостей проверяемо свободных фото
  не существует, не трать время на попытки без явного PD/CC-тега на конкретном файле. Отдельно:
  драматизированный/спекулятивный текст («тайна», «секрет», непроверенные утверждения) про живых
  конкретных людей — это уже не про копирайт, а риск клеветы/репутации; про живых пиши только
  нейтральные проверяемые факты, драматизацию — только про фигуры, о которых уже нет кому судиться.
- **Причина (прямая директива пользователя 2026-06-26):** пользователь НЕ хочет юридического риска
  (DMCA-удаление, copyright-страйк = удаление канала, теоретически иск) — **даже при том, что каналы
  не монетизируются**. Аргумент «просто хочу больше просмотров» этого НЕ отменяет: на копирайт не забивать.
- Это подтверждает и ужесточает уже имеющиеся правила (`docs/pack-generation.md`: visual-riddles/мемы —
  «только CC0/PD/лицензионно-чистое»). Любое послабление «по ходу диалога» этим правилом ОТМЕНЯЕТСЯ.
  Не уверен в лицензии источника — спроси пользователя, не качай.
- **Мемы — источник (прямое пожелание пользователя 2026-06-26):** generic **Pexels-сток для мемов как
  САМ мем НЕ использовать** (Pexels-фото = «просто картинки», не мемы). Источник мемов — основной пак
  «Генератор мемов» / реальные мем-шаблоны (`docs/pack-generation.md`). Pexels ОК для других дек и как
  нейтральный фон-носитель уже сделанных insert-slot шаблонов — но НЕ как новый источник самих мемов.

## Super-admin pack rules (MANDATORY)
- **MGS-паки и MGS-шаблоны не относятся к armen/super-admin.** Это отдельный клиентский контент:
  не использовать его как базу, донор, fallback, источник шаблонов или активную деку для super-admin,
  если пользователь явно не попросил отдельную MGS-задачу.
- Для super-admin не добавлять новые `plain background + text` шаблоны. Нужен тематический визуальный
  слой: generated/PD/Pexels фон, предмет, сцена, рамка, орнамент или другой осмысленный дизайн-элемент.
  Исключение — анекдоты: простая бумага/стикер допустимы, если текст крупный и читаемый, а emoji/GIF
  не перекрывает текст и не выглядит как плашка канала.
- После крупных правок super-admin паков запускай локальный аудит:
