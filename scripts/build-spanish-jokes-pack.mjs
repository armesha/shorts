...
const unsafeRules = [
  ...
  ["religion", /\b(dios\w*|virgen|santo|santa|san|cura|fraile|frailes|monja|iglesia|misa|serm[oó]n\w*|predicador\w*|diabl\w*|demonio|infierno|pecado|cielo|cristo|cristian\w*|religi[oó]n\w*|ermita\w*|ermitañ\w*|devot\w*|penitent\w*|\w*disc[ií]pul\w*|farise\w*|evangelio|evangelista|apocalypsis|apocalipsis|ap[oó]stol|obispo|papa|sacrist[aá]n|sacerdote|confesor\w*|franciscano|dominico|limosna|[aá]ngel\w*|bruja|prodigio|ruegos|orar|oraci[oó]n|f[eé])\b/i],
  ...
  ["politics_crime_or_authority", /\b(rey|reina|reyes|gobierno|pol[ií]tic|ministro|c[aá]rcel|preso|prisioner\w*|reo\w*|litigante\w*|jurisconsult\w*|rob\w*|hurtad\w*|ladr[oó]n|ladrones|delito|sentencia|juez|tribunal|guardia|polic[ií]a|justicia|conde|duque|marqu[eé]s|pr[ií]ncipe|cromwel\w*|falsari\w*|falsos?|adulador\w*|coronel|capit[aá]n|general|victoria|emperador|vespasiano|impuesto\w*|imperio|sublev\w*|procesab\w*|confiscab\w*|guerra|maestre|sable|pasaporte|soberano|ilustr[ií]simo)\b/i],
  ...
  ["children_family_or_sexist_setup", /\b(niñ[oa]s?|muchach[oa]s?|colegial|joven|j[oó]venes|hijas?|hijos?|hijuel\w*|madre|mam[aá]|padre|familia|hermano|hermana|mujer|mujeres|muger\w*|señora|señoras|esposa|marido|viuda|vieja|viejo|nuera|prima|sobrina|casad[oa]s?|casar\w*|cases|solter[ao]s?|novi[ao]s?|criad[ao]s?|esclav\w*|amo|ama|aldean[oa]s?)\b/i],
  ["extended_family_or_gender_setup", /\b(padres?|madres?|herman[ao]s?|familias?|se[nñ]orit\w*|damas?|jovencit\w*)\b/i],
...
function buildReport({ selected, targetTemplates, acceptedCount, dedupedCount, rejected, selectedFlags, profile }) {
  return {
    ...
    targetTemplates,
    policy:
      "Local deterministic extraction from public-domain Spanish books. The filter rejects adult/sexual, violence/death, protected-class or nationality stereotypes, religion, alcohol/drugs, politics/crime/authority, medical/body, children/family/sexist setups, coarse insults, scan headers, OCR artifacts, verse fragments, and truncated snippets. No LLM text cleanup/adaptation was used for this production pass.",
