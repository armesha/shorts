...
if (!existsSync(REPORT_PATH)) {
  console.error(`missing safety audit report: ${REPORT_PATH}`);
  console.error("run: node --experimental-sqlite --import tsx src/scripts/audit-armen-packs-safety.mjs");
  process.exit(1);
}
...
const flaggedIds = [
  ...new Set(
    (report?.queue?.flags ?? [])
      .map((flag) => Number(flag?.videoId))
      .filter((id) => Number.isInteger(id) && id > 0),
  ),
];

if (!flaggedIds.length) {
  console.log("armen flagged queued videos: 0");
  process.exit(0);
}
...
const rows = db
  ...
  .all(USERNAME, ...flaggedIds);

const flagsById = new Map((report?.queue?.flags ?? []).map((flag) => [Number(flag.videoId), flag]));
console.log(`armen flagged queued videos: ${rows.length}`);
for (const row of rows) {
  const flag = flagsById.get(Number(row.id));
  console.log(`- ${row.id} account=${row.account_id} deck=${row.deck} check=${flag?.check ?? "unknown"} title=${row.title}`);
}
...
const backupPath = resolve(BACKUP_DIR, `armen-flagged-queue-videos-${stamp}.json`);
mkdirSync(dirname(backupPath), { recursive: true });
writeFileSync(backupPath, JSON.stringify({ generatedAt: new Date().toISOString(), flags: report?.queue?.flags ?? [], rows }, null, 2));
