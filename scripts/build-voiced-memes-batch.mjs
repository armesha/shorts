...
try {
  for (let index = 0; index < manifest.length; index += 1) {
    ...
    try {
      ...
      run(process.execPath, ["scripts/build-voiced-meme-short.mjs", "--meme", image, "--avatar", avatar, "--avatar-background", style.backgroundFile, "--ring-color", style.ringColor, "--history", resolve(outDir, "gameplay-history.json"), "--output", base]);
